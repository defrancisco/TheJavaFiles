...
public class Actividad1 {
    ...
    public static int mochila(int[] pesos, int[] valores, int capacidad){
        ...
        //relleno la tabla DP
        for(int i = 1; i <= n; i++){ //iteras sobre cada objeto
            for(int j = 1; j<= capacidad; j++){ //iteras sobre cada capacidad
                //va a agregar hasta la capacidad 
                if(pesos[i-1] <= j){ // si el peso del objeto actual es menor o igual a la capacidad actual (j) 
                    //decidimos incluir este objeto
                    // Si el peso del objeto i-ésimo es menor o igual a la capacidad w
                    dp[i][j] = Math.max(valores[i - 1] + dp[i - 1][j - pesos[i - 1]], dp[i - 1][j]);
                } else{
                    // Si el peso del objeto i-ésimo es mayor que la capacidad j
                    //copiamos el valor de la fila anterior
                    dp[i][j] = dp[i-1][j];
                }
            }
        }
